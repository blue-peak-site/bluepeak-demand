...
export default function Layout() {
  ...
  const isRequestStormActivationPage =
    location.pathname === '/request-storm-activation';
  const isStormActivationPage = location.pathname === '/storm-activation';
  const isInsightsPage = location.pathname === '/insights';
  const isInsightArticlePage = location.pathname.startsWith('/insights/');

  const isBlogPage = isInsightsPage || isInsightArticlePage;
  ...
  const headerClassName = (() => {
    ...
    if (isAuditExamplePage) {
      return isScrolled
        ? 'bg-[#020408]/80 backdrop-blur-md border-b border-white/5 py-4'
        : 'bg-[#020408] py-6';
    }
    ...
    if (isRequestStormActivationPage || isStormActivationPage) {
      return isScrolled
    ...
    if (isBlogPage) {
      return isScrolled
        ? 'bg-bp-bg/70 backdrop-blur-xl border-b border-white/5 py-4'
        : 'bg-bp-bg py-6';
    }
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-bp-bg">
      ...
      {/* Header */}
      <header
        className={`fixed top-0 w-full z-50 transition-all duration-300 ${headerClassName}`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
              ...
              return (
                <Link
                  ...
                  className={`text-sm font-medium transition-all relative py-2 ${
                    isActive
                      ? 'text-bp-white'
                      : 'text-bp-support hover:text-bp-white'
                  }`}
                >
                  {link.name}
                  {isActive && (
            ...
            <Link
              to="/free-audit"
              className="px-5 py-2.5 rounded-lg bg-bp-cta text-white font-semibold text-sm hover:bg-bp-cta/90 transition-all shadow-[0_0_20px_rgba(30,167,255,0.3)] hover:shadow-[0_0_30px_rgba(30,167,255,0.5)]"
            >
              Free Audit
            </Link>
          </nav>
        ...
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 w-full bg-bp-bg/95 backdrop-blur-xl border-b border-white/10 p-6 flex flex-col gap-4 md:hidden"
          >
            {navLinks.map((link) => (
              <Link
                key={link.name}
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 mb-12">
            ...
            <div className="lg:col-span-5">
              ...
              <p className="text-bp-support text-base leading-relaxed max-w-sm opacity-60">
                Premium local demand intelligence platform for storm-damage
                roofing companies. We turn Google Maps into a high-converting
                inspection engine.
              </p>
            </div>
            ...
            <div className="lg:col-span-7 grid grid-cols-2 md:grid-cols-3 gap-12">
              <div>
                <h4 className="text-bp-white font-bold mb-6 text-xs uppercase tracking-[0.3em] opacity-40">
                  Platform
                </h4>
                <ul className="flex flex-col gap-5">
                  <li>
                    <Link
                      to="/stormrank"
                      className="text-bp-support hover:text-bp-cta transition-colors text-sm font-medium"
                    >
                      StormRank System
                    </Link>
                  </li>
              ...
              <div className="col-span-2 md:col-span-1">
                ...
                <ul className="flex flex-col gap-5">
                  <li>
                    <a
                      href="mailto:yasmim@bluepeakdemand.com"
                      className="text-bp-support hover:text-bp-cta transition-colors text-sm font-medium"
                    >
          ...
          <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-8">
              <a
                href="#"
                className="text-[10px] font-mono text-bp-support/20 hover:text-bp-support transition-colors uppercase tracking-[0.4em]"
              >
                Privacy
              </a>
              <a
